...
Revision ID: 09610e4e4f79
Revises: 
Create Date: 2026-01-25 03:56:57.346737
...
def upgrade() -> None:
    ...
    op.create_table('host_subscriptions',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('user_id', sa.Integer(), nullable=False),
    sa.Column('status', sa.Enum('ACTIVE', 'EXPIRED', 'CANCELLED', 'TRIAL', name='subscriptionstatus'), nullable=False),
    sa.Column('tier', sa.Enum('BASIC', 'PREMIUM', 'ENTERPRISE', name='subscriptiontier'), nullable=False),
    ...
    op.create_index(op.f('ix_host_subscriptions_external_customer_id'), 'host_subscriptions', ['external_customer_id'], unique=False)
    op.create_index(op.f('ix_host_subscriptions_external_subscription_id'), 'host_subscriptions', ['external_subscription_id'], unique=True)
    op.create_index(op.f('ix_host_subscriptions_id'), 'host_subscriptions', ['id'], unique=False)
    op.create_index(op.f('ix_host_subscriptions_user_id'), 'host_subscriptions', ['user_id'], unique=True)
    ...
    sa.Column('buy_in_info', sa.String(), nullable=True),
    ...
    # Create index only if it doesn't exist (handles partial migration case)
    op.execute("""
    ...
    # Note: PostGIS tiger geocoder system tables are intentionally not dropped
    # These tables are part of the PostGIS extension and should not be managed by Alembic
...
def downgrade() -> None:
    ...
    op.drop_table('rooms')
    op.drop_index(op.f('ix_host_subscriptions_user_id'), table_name='host_subscriptions')
    op.drop_index(op.f('ix_host_subscriptions_id'), table_name='host_subscriptions')
    op.drop_index(op.f('ix_host_subscriptions_external_subscription_id'), table_name='host_subscriptions')
    op.drop_index(op.f('ix_host_subscriptions_external_customer_id'), table_name='host_subscriptions')
